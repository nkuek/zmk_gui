...
static void draw_middle(lv_obj_t *widget, const struct status_state *state) {
    lv_obj_t *bongocat = lv_obj_get_child(widget, 1);
...
ZMK_LISTENER(widget_position_state, position_state_changed_cb);
ZMK_SUBSCRIPTION(widget_position_state, zmk_position_state_changed);
